...
app.use("/server/data", (req, res) => {
  res
    .status(403)
    .json({ message: "Akses langsung ke database tidak diizinkan." });
});
...
function validateApiSignature(req, res, next) {
  ...
  if (!signature || !timestamp) {
    ...
    return res
      .status(403)
      .json({ message: "Akses ditolak. Signature diperlukan." });
  }
  ...
  if (isNaN(reqTime) || Math.abs(now - reqTime) > SIGNATURE_MAX_AGE_SECONDS) {
    ...
    return res
      .status(403)
      .json({ message: "Akses ditolak. Timestamp kadaluwarsa." });
  }
  ...
  if (signature !== expectedSignature) {
    ...
    return res
      .status(403)
      .json({ message: "Akses ditolak. Signature tidak valid." });
  }
...
function rateLimiter(req, res, next) {
  ...
  if (entry.count > RATE_LIMIT_MAX) {
    console.warn(
      `[Rate Limit] IP ${ip} exceeded ${RATE_LIMIT_MAX} requests/min`,
    );
    res.setHeader(
      "Retry-After",
...
function authenticateFirebaseToken(req, res, next) {
  ...
  if (!token) {
    return res
      .status(401)
      .json({ message: "Autentikasi diperlukan. Token tidak ditemukan." });
  }
...
function authenticateAdminToken(req, res, next) {
  authenticateFirebaseToken(req, res, () => {
    if (
      !req.user ||
      !req.user.email ||
      req.user.email.toLowerCase() !== "planetkapal@gmail.com"
    ) {
      console.warn(
        `[Unauthorized Access] Non-admin user ${req.user ? req.user.email : "unknown"} attempted to modify/delete CCTV`,
      );
      return res
        .status(403)
        .json({
          message:
            "Akses ditolak. Hanya admin (planetkapal@gmail.com) yang diizinkan melakukan tindakan ini.",
        });
    }
    next();
  });
...
app.post("/api/cctvs", authenticateFirebaseToken, (req, res) => {
  ...
  if (
    ...
    return res
      .status(400)
      .json({ message: "Seluruh parameter utama CCTV harus diisi." });
  }
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Gagal menyimpan data ke database server." });
  }
});
...
app.put("/api/cctvs/:id", authenticateAdminToken, (req, res) => {
  ...
  try {
    fs.writeFileSync(dbPath, JSON.stringify(cctvData, null, 2), "utf-8");
    console.log(
      `[Database] CCTV Updated: ${updatedCctv.name} by ${req.user.email}`,
    );
...
app.delete("/api/cctvs/:id", authenticateAdminToken, (req, res) => {
  ...
  try {
    ...
    res.json({
      success: true,
      message: `CCTV ${deletedName} berhasil dihapus.`,
    });
  } catch (err) {
    console.error("[Database Error] Failed to write database:", err.message);
    res
  ...
  try {
    ...
    if (!streamRes.ok) {
      return res
        ...
        .send(`Failed to get stream URL for camera ${id}`);
    }
...
app.get("/api/bontang-stream/:cameraId/stream.mp4", (req, res) => {
  ...
  res.redirect(
    `https://i-see.iconpln.co.id/ckexo/media?session=${bontangSession}&cameraId=${cameraId}&format=fmp4&frames=all&media=video&t=live`,
  );
});
...
app.get("/api/bontang-snapshot/:cameraId", (req, res) => {
  const { cameraId } = req.params;
  res.redirect(
    `https://i-see.iconpln.co.id/ckexo/media?session=${bontangSession}&cameraId=${cameraId}&format=jpeg&frames=1&media=image`,
  );
});
...
app.get("/api/kutim-stream/:streamKey", async (req, res) => {
  ...
  try {
    ...
    console.log(
      `[Kutim Stream Wakeup] ${streamKey} -> Status: ${triggerRes.status}`,
    );
  } catch (err) {
      ...
      on: {
        proxyReq: (proxyReq, req) => {
          ...
          if (options.userAgent)
            proxyReq.setHeader("User-Agent", options.userAgent);
          if (options.cookies) proxyReq.setHeader("Cookie", options.cookies);
        },
        proxyRes: (proxyRes, req, res) => {
          console.log(
            `[Proxy Response] ${req.method} ${req.originalUrl} -> Target Status: ${proxyRes.statusCode}, Content-Type: ${proxyRes.headers["content-type"]}`,
          );
...
createCctvProxy(
  ...
  "https://sliceseribuwajah.bandarlampungkota.go.id",
  {
...
createCctvProxy(
  ...
  "https://pelindung.bandung.go.id:3443",
);
...
if (!process.env.VERCEL) {
  app.listen(PORT, () => {
    console.log(
      `[Server] Express backend server is running on http://localhost:${PORT}`,
    );
  });
}
