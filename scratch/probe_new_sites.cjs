...
async function fetchUrl(url, name) {
  ...
  try {
    ...
    const keywords = ['iframe', 'video', 'm3u8', 'hls', 'flv', 'rtsp', 'stream', 'cctv', 'lat', 'lng', 'geojson', 'marker'];
...
async function main() {
  await fetchUrl('https://dishub.bandungkab.go.id/cctv/', 'bandungkab');
  await fetchUrl('https://atcs.bandungbaratkab.go.id/', 'bandungbarat');
  await fetchUrl('https://itscctv-dishub.bogorkab.go.id/', 'bogorkab');
}
